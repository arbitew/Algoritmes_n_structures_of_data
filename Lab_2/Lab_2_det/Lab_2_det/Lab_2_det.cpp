...
void quickSort(std::vector<T>& arr, int left, int right, int& comparisons, int& swaps, int& call, int& deep, int& maxDeep) {
    ...
    if (arr[left] > arr[(left + right) / 2]) {
        ...
        else if (arr[left] > arr[right]) std::swap(arr[(left + right) / 2], arr[right]);
        else std::swap(arr[(left + right) / 2], arr[left]);
    }
    else {
        if (arr[left] > arr[right]) std::swap(arr[(left + right) / 2], arr[left]);
        else if (arr[(left + right) / 2] > arr[right]) std::swap(arr[(left + right) / 2], arr[right]);
    }
...
void quickSort(std::vector<T>& arr) {
    ...
    std::cout << comparisons << " " << swaps << " " << call << " " << maxDeep << " ";
}
